...
export const guestController = {
  async join(req: Request, res: Response) {
    try {
      ...
      const guest = await guestService.joinEvent(req.userId!, req.params.eventId as string, { allergies, followersCount });
      res.status(201).json(guest);
    } catch (err: any) {
      const status = err.message.includes('not found') ? 404 : err.message.includes('Already') ? 409 : 400;
